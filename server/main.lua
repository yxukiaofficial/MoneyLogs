...
Citizen.CreateThread(function()
    while (Config.SendLogByTime.enable) do
        ...
        TriggerEvent("MoneyLogs:server:sendLog") --Trigger sending log event
    end
end)
...
QBCore.Commands.Add(Lang:t("command.name"), Lang:t("command.help"), {}, false, function()
    if (Config.OnlyTopRichest.enable) then
        TriggerEvent("MoneyLogs:server:getTopPlayerMoney", Config.LogMessageType)
    else
        TriggerEvent("MoneyLogs:server:getAllPlayerMoney", Config.LogMessageType)
    end
end, 'admin')

--Event for TOP RICHEST ONLY
RegisterNetEvent("MoneyLogs:server:getTopPlayerMoney", function(type)
    if (type == "full") then
...
RegisterNetEvent("MoneyLogs:server:getAllPlayerMoney", function(type)
    if (type == "full") then
...
RegisterNetEvent("MoneyLogs:server:sendLog", function()
    if (Config.OnlyTopRichest.enable) then
        TriggerEvent("MoneyLogs:server:getTopPlayerMoney", Config.LogMessageType)
    else
        TriggerEvent("MoneyLogs:server:getAllPlayerMoney", Config.LogMessageType)
